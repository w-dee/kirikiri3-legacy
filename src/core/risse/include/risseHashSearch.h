...
#define RISSE_HS_DEFAULT_HASH_SIZE 64
#define RISSE_HS_HASH_USING	0x1
#define RISSE_HS_HASH_LV1	0x2
...
{
//---------------------------------------------------------------------------
// tRisseHashFunc : Hash function object
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
template <>
class tRisseHashFunc<ttstr> // a specialized template of tRisseHashFunc for ttstr
{
public:
	static risse_uint32 Make(const ttstr &val)
	{
		if(val.IsEmpty()) return 0;
		const risse_char *str = val.c_str();
		risse_uint32 ret = 0;
		while(*str)
		{
			ret += *str;
...
//---------------------------------------------------------------------------
template <>
class tRisseHashFunc<risse_char *> // a specialized template of tRisseHashFunc for risse_char
{
public:
	static risse_uint32 Make(const risse_char *str)
	{
		if(!str) return 0;
		risse_uint32 ret = 0;
		while(*str)
...
//---------------------------------------------------------------------------
// tRisseHashTable : a simple implementation of Chain Hash algorithm
//---------------------------------------------------------------------------
/*
	Not that ValueT must implement both "operator =" and a copy constructor.
*/
template <typename KeyT, typename ValueT, typename HashFuncT = tRisseHashFunc<KeyT>,
	risse_int HashSize = RISSE_HS_DEFAULT_HASH_SIZE>
class tRisseHashTable
{
private:
	struct element
	{
...
//		{ elm = ref.elm; }
	...
	static risse_uint32 MakeHash(const KeyT &key)
	...
	void AddWithHash(const KeyT &key, risse_uint32 hash, const ValueT &value)
	{
		...
		// lv1 used ?
		if(!(lv1->Flags & RISSE_HS_HASH_USING))
		{
			// lv1 is unused
...
#endif
		element *lv1 = Elms + hash % HashSize;
		if(lv1->Flags & RISSE_HS_HASH_USING && hash == lv1->Hash)
		{
			if(key == *(KeyT*)lv1->Key)
			{
				// delete lv1
	...
	risse_int ChopLast(risse_int count)
	{
	...
	risse_uint GetCount() const { return Count; }

private:
	void InternalClear()
	{
		for(risse_int i = 0; i < HashSize; i++)
		{
			// delete all items
			...
			if(Elms[i].Flags & RISSE_HS_HASH_USING)
			{
				Destruct(Elms[i]);
			}
		}
	...
	void InternalInit()
	{
		...
		for(risse_int i = 0; i < HashSize; i++)
		{
			Elms[i].Flags = RISSE_HS_HASH_LV1;
			Elms[i].Prev = NULL;
			Elms[i].Next = NULL;
		}
	}
	...
	void DeleteByElement(element *elm)
	{
		CheckDeletingElementOrder(elm);
		Destruct(*elm);
		if(elm->Flags & RISSE_HS_HASH_LV1)
		{
			// lv1 element
			// nothing to do
		}
		...
		if(!(elm->Flags & RISSE_HS_HASH_LV1)) delete elm;
	}

	const element * InternalFindWithHash(const KeyT &key, risse_uint32 hash) const
	{
		// find key ( hash )
#ifdef RISSE_HS_DEBUG_CHAIN
		hash = 0;
		...
		if(hash == lv1->Hash && lv1->Flags & RISSE_HS_HASH_USING)
		{
...
//---------------------------------------------------------------------------
// tRisseHashCache : cache algorithm via tRisseHashTable
//---------------------------------------------------------------------------
template <typename KeyT, typename ValueT, typename HashFuncT = tRisseHashFunc<KeyT>,
	risse_int HashSize = RISSE_HS_DEFAULT_HASH_SIZE >
class tRisseHashCache : public tRisseHashTable<KeyT, ValueT, HashFuncT, HashSize>
{
	typedef tRisseHashTable<KeyT, ValueT, HashFuncT, HashSize> inherited;

	risse_uint MaxCount;

public:
	tRisseHashCache(risse_uint maxcount) { MaxCount = maxcount; }
	...
	void SetMaxCount(risse_uint maxcount)
	{
		MaxCount = maxcount;
		if(inherited::GetCount() > MaxCount)
		{
	...
	risse_uint GetMaxCount() { return MaxCount; }
};
//---------------------------------------------------------------------------
} // namespace Risse
//---------------------------------------------------------------------------
