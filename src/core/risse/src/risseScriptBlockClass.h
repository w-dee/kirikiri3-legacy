...
protected:
	...
	/**
	 * LinesToPosition の内容を作成する
	 */
	void CreateLinesToPositionArary() const;

	/**
	 * LinesToPosition の内容が作成されていなければ作成する
	 */
...
public:
	...
	/**
	 * スクリプトの内容を得る	@return スクリプトの内容
	 */
	const tString & GetScript() const { return Script; }

	/**
	 * スクリプトブロックの名称を得る	@return スクリプトブロックの名称
	 */
	const tString & GetName() const { return Name; }

	/**
	 * スクリプトのコードポイント位置から行/桁位置への変換を行う
	 * @param pos	コードポイント位置
	...
	/**
	 * スクリプト上の位置から "%1 at %2" 形式のメッセージを組み立てる
	 * @param pos		スクリプト上の位置
	 * @param message	メッセージ
	 * @return	組み立てられたメッセージ
	 */
