...
/*!
	...
	uta では 32bit を 8bit ずつにわけて それぞれが X0,Y0,X1,Y1 となり、
	32x32中の任意の座標を表している。
	ここの実装では 16bitを 3bit 5bit 3bit 5bit にわけてそれぞれを X0,Y0,X1,Y1
	とし、32x32中の任意の座標を表す。X に関しては 4 ピクセル数単位の値となる。
	最近のプロセッサのメモリアクセス方法を考えると、横方向の粒度は高くても
	しょうがない。

	座標点は矩形に対してinclusive である。
	たとえば (X0,Y0,X1,Y1) = (7,31,0,0) は (0,0)-(32,32) の矩形(つまり32x32
	全域)を表す。
