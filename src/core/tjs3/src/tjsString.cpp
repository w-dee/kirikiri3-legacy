...
//---------------------------------------------------------------------------
bool tTJSString::EndsWith(const tjs_char *string) const
{
	if(!Ptr)
	{
	...
	tjs_size string_len = TJS_strlen(string);
	tjs_size this_len = Ptr->GetLength();

	if(string_len > this_len) return false; // string が この文字列よりも長いので偽

	return !TJS_strcmp(string, *Ptr + this_len - string_len);
}
//---------------------------------------------------------------------------
