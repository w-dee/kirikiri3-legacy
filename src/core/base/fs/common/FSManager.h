...
public:
	...
	virtual bool OnFile(const tRisseString & filename) = 0;
	virtual bool OnDirectory(const tRisseString & dirname) = 0;
};
//---------------------------------------------------------------------------
...
public:
	...
	virtual size_t GetFileListAt(const tRisseString & dirname,
		tRisaFileSystemIterationCallback * callback) = 0; //!< ファイル一覧を取得する
	virtual bool FileExists(const tRisseString & filename) = 0; //!< ファイルが存在するかどうかを得る
	virtual bool DirectoryExists(const tRisseString & dirname) = 0; //!< ディレクトリが存在するかどうかを得る
	virtual void RemoveFile(const tRisseString & filename) = 0; //!< ファイルを削除する
	virtual void RemoveDirectory(const tRisseString & dirname, bool recursive = false) = 0; //!< ディレクトリを削除する
	virtual void CreateDirectory(const tRisseString & dirname, bool recursive = false) = 0; //!< ディレクトリを作成する
	virtual void Stat(const tRisseString & filename, tRisaStatStruc & struc) = 0; //!< 指定されたファイルの stat を得る
	virtual tRisseBinaryStream * CreateStream(const tRisseString & filename, risse_uint32 flags) = 0; //!< 指定されたファイルのストリームを得る
};
//---------------------------------------------------------------------------
	...
	protected depends_on<tRisaRisseScriptEngine>
{
	...
	tRisseHashTable<tRisseString, tFileSystemInfo> MountPoints; //!< マウントポイントのハッシュ表
	tRisseString CurrentDirectory; //!< カレントディレクトリ (パスの最後に '/' を含む)
...
public:
	...
	void Mount(const tRisseString & point, iRisseDispatch2 * fs_risseobj);

	//! @brief		ファイルシステムをアンマウントする
	//! @param		point マウントポイント
	//! @note		メインスレッド以外から呼び出さないこと
	void Unmount(const tRisseString & point);
	...
	//! @param		path 正規化したいパス
	//! @return		正規化されたパス
	tRisseString NormalizePath(const tRisseString & path);
...
private:
	...
	size_t InternalGetFileListAt(const tRisseString & dirname,
	...
	boost::shared_ptr<tRisaFileSystem> GetFileSystemAt(const tRisseString & fullpath, tRisseString * fspath = NULL);

	//! @brief		「ファイルシステムが指定されたパスはない」例外を発生させる
	//! @param		filename  マウントポイント
	//! @note		この関数は例外を発生させるため呼び出し元には戻らない
	static void ThrowNoFileSystemError(const tRisseString & filename);
...
public:
	...
	static void SplitExtension(const tRisseString & in, tRisseString * other, tRisseString * ext);
	...
	//! @note		in と そのほかのパラメータに同じ文字列を指定しないこと
	static void SplitPathAndName(const tRisseString & in, tRisseString * path, tRisseString * name);

	//! @brief		パス名の最後のパスデリミタ ('/') を取り去る ( /path/is/here/ を /path/is/here にする )
	//! @param		path パス
	static void TrimLastPathDelimiter(tRisseString & path);

	//! @brief		拡張子を切り落とす
	//! @param		in 処理したいファイル名
	//! @return		拡張子が取り落とされたファイル名
	static tRisseString ChopExtension(const tRisseString & in);

	//! @brief		拡張子を取り出す
	//! @param		in 処理したいファイル名
	//! @return		拡張子(ドットも含む; 拡張子が無い場合は空文字)
	static tRisseString ExtractExtension(const tRisseString & in);

	//! @brief		ファイル名を返す (パスの部分を含まない)
	//! @param		in 処理したいファイル名
	//! @return		ファイル名
	static tRisseString ExtractName(const tRisseString & in);

	//! @brief		ファイル名のパス名を返す
	//! @param		in 処理したいファイル名
	//! @return		パス名
	static tRisseString ExtractPath(const tRisseString & in);

	//! @brief		フルパスを得る
	tRisseString GetFullPath(const tRisseString & in) { return NormalizePath(in); }

	//! @brief		現在の作業ディレクトリを得る
	//! @return		作業ディレクトリ
	const tRisseString & GetCurrentDirectory();

	//! @brief		作業ディレクトリを設定する
	//! @param		dir   作業ディレクトリ
	//! @note		実際にそのディレクトリが存在するかどうかのチェックは行わない
	void SetCurrentDirectory(const tRisseString &dir);
};
//---------------------------------------------------------------------------
