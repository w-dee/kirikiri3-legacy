...
{
	if(ch >= 0x100) return false; else return isspace(ch);
}
//---------------------------------------------------------------------------
...
{
	if(ch >= 0x100) return false; else return isdigit(ch);
}
//---------------------------------------------------------------------------
...
//! @param		ch		文字
//! @return		アルファベットの時に真
//! @note		ここでは 0x100 以上の文字はすべてアルファベットと同じ扱いとする
//---------------------------------------------------------------------------
static bool inline Risse_iswalpha(risse_char ch)
{
	if(ch >= 0x100) return true; else return isalpha(ch);
}
//---------------------------------------------------------------------------
...
{
	while(*ptr && Risse_iswspace(*ptr)) ptr ++;
	return *ptr;
}
//---------------------------------------------------------------------------
...
bool tRisseLexer::StringMatch(const risse_char *wrd, bool isword)
{
	...
	if(isword)
	{
		if(Risse_iswalpha(*Ptr) || *Ptr == RISSE_WC('_'))
		...
		if(*Ptr == RISSE_WC('\\'))
		{
			...
			if(*Ptr == RISSE_WC('x') || *Ptr == RISSE_WC('X'))
			{
				...
				while((num = RisseHexNum(*Ptr)) != -1 &&
					(size_t)count < (sizeof(risse_char)*2))
				{
					code <<= 4;
					code += num;
					count ++;
