...
loop:
	{
		bool b= flags & 1;
		flags >>= 1;
		if(RISSE_UNLIKELY(!flags)) goto getmore;
		...
		if(b)
		{
			...
			in += 2;
			if(RISSE_UNLIKELY(mlen == 15)) mlen += 0[in++];
			mlen += 3;
/*
...
*/
	...
	if(RISSE_LIKELY(in < inlim)) goto loop;
...
//---------------------------------------------------------------------------
static void TLG6DecodeGolombValues(
	RISSE_RESTRICT risse_int8 *pixelbuf,
	risse_int pixel_count,
	RISSE_RESTRICT risse_uint8 *bit_pool)
{
