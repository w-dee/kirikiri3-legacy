...
void smbPitchShifter::filter(float pitchShift, long numSampsToProcess,
...
*/
{
	float *indata = gInputBuffer;
	float *outdata = gOutputBuffer;
...
void smbPitchShifter::fft(float *fftBuffer, long fftFrameSize, long sign)
/* 
...
double smbPitchShifter::atan2(double x, double y)
{
  double signx;
  if (x > 0.) signx = 1.;  
  else signx = -1.;
