...
public:
	...
	/**
	 * 共有されたレジスタのマップに変数名を追加する
	 * @param name	変数名
	 */
	void AddSharedRegNameMap(const tString & name);

	/**
	 * 共有されたレジスタのマップにバインディング変数名とレジスタを追加する
	 * @param name		変数名
	 * @param nestlevel	そのレジスタのネストレベル
	 * @param regnum	レジスタ番号
	...
	/**
	 * 指定されたネストレベルに対する共有されたレジスタの個数を得る
	 * @return	共有されたレジスタの個数
	...
	/**
	 * VariableMapForChildren 用の変数を登録する
	 * @param var	変数
	 * @param name	変数名
	 */
	...
	/**
	 * VariableMapForChildren 内で変数を探す
	 * @param name	変数名
	 * @return	そのレジスタのインデックス
...
public:
	...
	/**
	 * これから置く命令コードがソースコード上のどの位置に対応するかを通知する
	 * @param pos	ソースコード上の位置
	 */
	void SetSourceCodePosition(risse_size pos);

	/**
	 * これから置く命令コードがソースコード上のどの位置に対応するかを得る
	...
	/**
	 * Assignコードを置く(このメソッドは削除の予定;使わないこと)
	 * @param dest	変数コピー先変数
	 * @param src	変数コピー元変数
	 */
	...
	/**
	 * オブジェクトを Assign するコードを置く(AssignThis, AssignSuper等)
	 * @param dest	変数コピー先変数
	...
	/**
	 * 新しい正規表現オブジェクトを Assign するコードを置く
	...
	/**
	 * 新しいプロパティインスタンスを Assign するコードを置く
	 * @param dest		Assign先変数
	...
	/**
	 * 新しいモジュールインスタンスを Assign するコードを置く
	 * @param dest	Assign先変数
	...
	/**
	 * FuncCall あるいは New コード または TryFuncCall を置く
	 * @param dest		関数結果格納先
	 * @param func		関数を表す変数
	 * @param code		コード(種別)
	...
	/**
	 * dest = op(arg1, arg2, arg3) 系コードを置く
	 * @param op	オペレーションコード
	 * @param dest	結果を格納する変数
	 * @param arg1	パラメータ1
	 * @param arg2	パラメータ2
