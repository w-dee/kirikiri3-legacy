...
public:
	...
	/**
	 * 合併可能な変数のリストがまだ作成されていないようならば作成する
	...
	/**
	 * この変数がとりうる値の型を tVariant::tGuessType として返す
	...
	/**
	 * この変数が使用されている文のうち、死んでいるブロックに属する文を
	 * 削除する
	 */
	...
	/**
	 * この変数の型や定数に関するコメントを得る
	 */
	tString GetTypeComment() const;

	/**
	 * 各文のダンプの後ろにつけられるようなコメントを生成して返す
	 */
